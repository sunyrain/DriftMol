#!/usr/bin/env python3
"""Collect next-wave reviewer experiment results."""
...
DEFAULT_MANIFEST = ROOT / "configs" / "publication_ext" / "next_wave_manifest.json"
OUT_CSV = ROOT / "results" / "next_wave_results.csv"
OUT_STATUS = ROOT / "results" / "next_wave_status.json"
OUT_TEX = ROOT / "results" / "tables" / "tab_next_wave.tex"
...
def parallel_running_names(status_root: Path) -> set[str]:
    names: set[str] = set()
    for status_path in status_root.glob("parallel_runner_status_next_wave*.json"):
...
def main() -> None:
    ...
    print(f"Collected {len(payloads)} next-wave entries")
